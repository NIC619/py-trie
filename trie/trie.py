...
class BinaryTrie():
    ...
    def get(self, key):
        ...
        Keypath will be encoded into binary format first
        """
        return self._get(self.root_hash, encode_to_bin(key))

    def _get(self, node_hash, keypath):
        """
        Note: keypath should be in binary format(i.e. encoded by encode_to_bin())
        """
        # Empty trie
        if node_hash == BLANK_HASH:
            return None
        left_child, right_child, nodetype = parse_node(self.db[node_hash])
    ...
    def _set(self, node_hash, keypath, value):
        ...
        # node is a key-value node
            ...
            if keypath[:len(left_child)] == left_child:
                ...
                if subnode_hash == BLANK_HASH:
                    return BLANK_HASH
                #print(db[o])
                sub_left_child, sub_right_child, subnodetype = parse_node(self.db[subnode_hash])
        ...
        # node is a branch node
            ...
            if new_left_child == BLANK_HASH or new_right_child == BLANK_HASH:
                sub_left_child, sub_right_child, subnodetype = parse_node(
                    self.db[
                        new_left_child
